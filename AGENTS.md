...
生产 WinForms 上位机必须加载 `.flowruntime` 并通过 `FlowRunner` 执行，不打开 WPF 流程设计器。
...
这些节点源码命名空间保留为 `Vision.Flow.Nodes`，但编译产物属于 `Vision.Flow.Core.dll`。
...
Designer 默认注册 Core 基础节点；具体项目可以注入自己的 `NodeRegistry` 来提供设备/算法节点。
...
Adapter 包装现有上位机代码、真实设备 SDK 或测试桩。

Adapter 接口放在 `Flow.Core`。

真实设备集成、Fake Adapter、Demo 设备和项目专属节点应由具体上位机应用或单独项目实现，不放进 `Flow.Core` 或 `Designer.Wpf`。
...
- 新增注释必须使用中文。
- 注释说明职责边界、复杂流程、线程/队列约束、设备适配边界和不明显的业务规则。
- 注释不应逐行复述代码。
- 公共模型类和关键属性应补充中文注释，说明用途、运行态/设计态边界、序列化影响、上位机集成含义和兼容性约束。
- 节点类型、端口名、数据类型、变量输出名、设置键、事件 Data 键、校验错误码、文件扩展名等领域字符串应集中定义。
...
- 每个功能点完成后，必须提交当前更改。
- 如果使用功能分支开发，功能分支提交完成后必须合并回主分支。
- 合并回主分支后必须确认构建、测试或相关验证无误。
- 合并确认无误后，必须删除已合并的功能分支。
