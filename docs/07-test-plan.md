...
- Serialization / Publish：Schema v2 round-trip、明确拒绝 v1、结构化 Setting Selector、发布后移除 view state、样例流程校验。
...
- Designer：配置项固定值/变量切换与常量保留、祖先变量候选范围、类型过滤、失效 Selector 保留、属性面板只读、节点库交互、拖拽、停止调试、按钮状态恢复、节点卡片运行状态显示。
- Variable Settings：常量/变量切换、上游来源限制、类型兼容、Token 与对象子路径解析、JSON 中不出现 `InputBindings`。
