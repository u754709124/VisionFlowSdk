...
- Node palette entries use single-click to select, double-click to add near the center of the current visible canvas viewport, and drag-drop to add at the drop position. Palette additions do not auto-connect to the previous node or inject hidden previous-node bindings.
...
- During Debug Run mode, each node card can show plain runtime status text above the card body. Waiting/unvisited nodes are greyed, running nodes receive a temporary highlighted border, completed nodes show success and elapsed time, stopped nodes show a stopped state with elapsed time, and failed/timeout nodes show status, elapsed time, and a compact reason with the full reason available from the tooltip.
- Node cards and canvas node layers opt into layout rounding, device-pixel snapping, display text formatting, and ClearType rendering to reduce blurry card text when the canvas is zoomed out.
